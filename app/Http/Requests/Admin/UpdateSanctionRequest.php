...
class UpdateSanctionRequest extends FormRequest
{
    ...
    {
        $id = $this->route('sanction')['id'];

        return [
            'criteria' => 'required|string|min:10|max:100|unique:sanctions,criteria,' . $id,
            'weight_from' => 'required|integer|unique:sanctions,weight_from,' . $id,
            'weight_to' => 'required|integer|unique:sanctions,weight_to,' . $id,
            'sanction' => 'required|string|min:10|max:90|unique:sanctions,sanction,' . $id,
        ];
    }
    ...
    protected function failedValidation(Validator $validator)
    {
        throw new HttpResponseException(
            Redirect::back()->withInput()->withErrors($validator->errors())->with('error', 'Data sanksi gagal diperbarui.')
        );
    }
}
