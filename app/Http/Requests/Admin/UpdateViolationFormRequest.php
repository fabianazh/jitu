...
class UpdateViolationFormRequest extends FormRequest
{
    /**
    ...
    {
        $id = $this->route('violationForm')['id'];

        return [
            'violation_category_id' => 'required|exists:violation_categories,id',
            'description' => 'required|string|max:120|unique:violation_forms,description,' . $id,
    ...
    protected function failedValidation(Validator $validator)
    {
        throw new HttpResponseException(
            Redirect::back()->withInput()->withErrors($validator->errors())->with('error', 'Bentuk pelanggaran gagal diperbarui.')
        );
    }
}
